#include <iostream>
#include <vector>
#include <iomanip>

// Original Shellcodes from shcode.h
unsigned char g_shellcode_stub[] = {
	0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90,
	0x66, 0x9C, 0x50, 0x51,
	0x52, 0x53, 0x55, 0x56, 0x57, 0x41, 0x50, 0x41, 0x51, 0x41, 0x52, 0x41,
	0x53, 0x41, 0x54, 0x41, 0x55, 0x41, 0x56, 0x41, 0x57, 0xE8, 0x1F, 0x00,
	0x00, 0x00, 0x41, 0x5F, 0x41, 0x5E, 0x41, 0x5D, 0x41, 0x5C, 0x41, 0x5B,
	0x41, 0x5A, 0x41, 0x59, 0x41, 0x58, 0x5F, 0x5E, 0x5D, 0x5B, 0x5A, 0x59,
	0x58, 0x66, 0x9D, 0xFF, 0x25, 0xBB, 0xFF, 0xFF, 0xFF
};

unsigned char g_shellcode_create_thread[] = {
	
};

unsigned char waiting_thd_hijack[] = {
	
};

void PrintEncrypted(const char* name, unsigned char* data, size_t size, unsigned char key) {
    std::cout << "unsigned char " << name << "[] = {\n\t";
    for (size_t i = 0; i < size; ++i) {
        std::cout << "0x" << std::hex << std::setw(2) << std::setfill('0') << (int)(data[i] ^ key);
        if (i < size - 1) std::cout << ", ";
        if ((i + 1) % 12 == 0) std::cout << "\n\t";
    }
    std::cout << "\n};\n\n";
}

int main() {
    unsigned char key = 0xAA; // Simple XOR key
    std::cout << "#pragma once\n\n";
    std::cout << "// Encrypted with XOR key: 0x" << std::hex << (int)key << "\n\n";
    
    PrintEncrypted("g_shellcode_stub", g_shellcode_stub, sizeof(g_shellcode_stub), key);
    PrintEncrypted("g_shellcode_create_thread", g_shellcode_create_thread, sizeof(g_shellcode_create_thread), key);
    PrintEncrypted("waiting_thd_hijack", waiting_thd_hijack, sizeof(waiting_thd_hijack), key);

    std::cout << "const unsigned char XOR_KEY = 0x" << std::hex << (int)key << ";\n\n";
    
    std::cout << "inline void DecryptShellcode(unsigned char* buffer, size_t size) {\n";
    std::cout << "    for (size_t i = 0; i < size; ++i) {\n";
    std::cout << "        buffer[i] ^= XOR_KEY;\n";
    std::cout << "    }\n";
    std::cout << "}\n";

    return 0;
}
